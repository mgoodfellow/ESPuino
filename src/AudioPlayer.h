...
typedef struct { // Bit field
	...
	uint16_t playUntilTrackNumber : 10; // Absolute track index at which uC goes to sleep (0 = inactive). Must cover
										// currentTrackNumber (9 bit) plus the configured track count, so it cannot be
										// narrower than that -- at 6 bit it silently wrapped past track 63.
